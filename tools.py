...
class Logger:
    ...
    def histogram(self, name, values):
        ...
        self._writer.add_histogram(name, values, self.step)

    def distribution(self, name, values):
        import numpy as np
        import torch
        ...
        self.scalar(f"{name}/max", float(np.max(values)))
        self.scalar(f"{name}/median", float(np.median(values)))
...
def get_modde_difficulty(task, seed=1234):
    import gym
    import random
    random.seed(seed)
    env = gym.make(f"ALE/{task.title().replace('_', '')}")
    ale = env.unwrapped.ale
    modes = ale.getAvailableModes()
    difficultys = ale.getAvailableDifficulties()
    return modes, difficultys
...
def simulate(
    ...
    if state is None:
        ...
        step, episode, done, length, obs, agent_state, reward, score = state
    ...
    while (steps and step < steps) or (episodes and episode < episodes):
        ...
        if done.any():
            ...
            for i in indices:
                if not offline_train or is_prefill:
                ...
                if not is_eval:
                    ...
                    print(f"Episode {envs[i].id} done: return={score:.1f}, length={length}")
                else:
    ...
    print(f'score: {score}, length: {length}')
    return (step - steps, episode - episodes, done, length, obs, agent_state, reward, score)
