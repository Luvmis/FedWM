...
class RSSM(nn.Module):
    ...
    @torch.no_grad()
    def collect_dynamics_stats(self, post, prior):
        """
        post, prior: (batch, time, ...)
        return dict of scalar stats
        """

        # -------- 1. KL(prior || posterior) --------
        kld = torchd.kl.kl_divergence
        dist = lambda x: self.get_dist(x)

        if self._discrete:
            kl = kld(dist(post), dist(prior))
        else:
            kl = kld(dist(post)._dist, dist(prior)._dist)

        kl_mean = kl.mean()

        # -------- 2. GRU hidden state health --------
        # deter: (batch, time, deter_dim)
        h = post["deter"]
        h_std = h.std(dim=(0, 1)).mean()  # scalar

        # -------- 3. Transition entropy --------
        # prior entropy ≈ dynamics uncertainty
        trans_entropy = self.get_dist(prior).entropy().mean()

        return {
            "kl": kl_mean.detach(),
            "h_std": h_std.detach(),
            "trans_entropy": trans_entropy.detach(),
        }
    ...
    def img_step(self, prev_state, prev_action, sample=True):
        ...
        if x.abs().max() > 20:
            print(f"⚠️ x 数值过大")
            print(f"  绝对值最大值: {x.abs().max().item():.2f}")
